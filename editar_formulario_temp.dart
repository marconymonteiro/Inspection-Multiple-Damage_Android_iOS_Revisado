...
class _EditarFormularioState extends State<EditarFormulario> {
  ...
  Future<void> _loadPhotos() async {
    ...
    if (formData?['damages'] != null) {
      for (var damage in formData!['damages']) {
        final description = damage['description'] ?? 'Sem descrição';
        final List<File> damagePhotos = [];

        if (damage['photos'] != null && damage['photos'] is List) {
          for (var url in damage['photos']) {
            try {
              final file = await _urlToFile(url);
              damagePhotos.add(file);
            } catch (e) {
              print('Erro ao baixar foto do dano: $e');
            }
          }
        }

        // Adiciona o dano com suas fotos à lista _damages
        _damages.add({
